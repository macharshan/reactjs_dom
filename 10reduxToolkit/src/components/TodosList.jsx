    ...
    {(todos.length > 0)? (
      ...
      {todos.map((todo)=>(
        <ul className='w-full p-4 text-lg bg-stone-300 rounded-lg flex flex-wrap justify-center items-center gap-4' key={todo.id} >
            <li className='basis-4 ml-4'><input type="checkbox" checked={todo.completed} onChange={()=>dispatch(toggleTodo(todo))} disabled={(updateStatus && todo.updateStatus) || (!todo.updateStatus && updateStatus)}/></li>
            <li className='grow'><h1 className={`${(todo.completed === true)? "line-through": "" }`}>{todo.text}</h1></li>
            <li><button className={`basis-4 mr-4 ${(!todo.completed && updateStatus)?"cursor-not-allowed":"cursor-pointer"} `}
                  onClick={()=>{
            ...
            <li><button className='basis-4 mr-4 cursor-pointer' 
                  onClick={()=>{
                    dispatch(updateTodoStat(todo))
                    setUpdateStatus(!updateStatus)}}
                  hidden={!(updateStatus && todo.updateStatus)}>❌</button></li>
            <li><button className={`basis-4 mr-4 ${(!todo.completed && updateStatus)?"cursor-not-allowed":"cursor-pointer"} `} 
